		      ...
		      btTrackerApp->SetStartTime (Seconds (0));
                      ...
                      for (uint32_t i = 0; i < newNodeCount; ++i)
                        {
                          ...
                          newNodeContainer.Add (m_topologyHelper->AddNodeToLastTopology ((*routerIt).second, deviceType, uplinkSpeed, downlinkSpeed, delay, false, false));
                        }
                    }
              ...
              else if (buffer == "other")
                {
                  ...
                  if (buffer == "nodes")
                    {
                      uint32_t newNodeCount;
                      std::string deviceType, uplinkSpeed = "", downlinkSpeed, delay;
                      NodeContainer newNodeContainer;
                      ...
                      std::cout << "		Added "<< newNodeCount << " other nodes to the topology." << std::endl;
                    }
                  else
                    {
                      NS_ABORT_MSG ("[line " << currentLine << "] Error: Can only add \"other nodes\" to the topology.");
                    }
                }
              else
                {
