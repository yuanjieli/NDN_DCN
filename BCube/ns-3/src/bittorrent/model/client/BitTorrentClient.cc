...
void BitTorrentClient::StartApplication ()
{
  ...
  m_ip = ipInterface->GetAddress (1,0).GetLocal ();
...
DataRate BitTorrentClient::GetBpsDownlink () const
{
  ...
  GetNode ()->GetDevice (0)->GetChannel ()->GetDevice (1)->GetAttribute ("DataRate", result);
  return result.Get ();
}
...
{
  DataRateValue result;
  GetNode ()->GetDevice (0)->GetChannel ()->GetDevice (0)->GetAttribute ("DataRate", result);
  return result.Get ();
}
