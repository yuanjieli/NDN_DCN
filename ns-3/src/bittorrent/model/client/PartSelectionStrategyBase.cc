...
PartSelectionStrategyBase::PieceNeeded PartSelectionStrategyBase::GeneratePieceNeeded (uint32_t pieceNeeded)
{
  ...
  while (currentOffset + m_myClient->GetRequestBlockSize () <= pieceLength - 1)
    {
      possibleBlocks.push_back (std::pair<uint32_t, uint32_t> (currentOffset, currentOffset + m_myClient->GetRequestBlockSize () - 1));
      currentOffset += m_myClient->GetRequestBlockSize ();
    }
  if (currentOffset < pieceLength)
    {
      possibleBlocks.push_back (std::pair<uint32_t, uint32_t> (currentOffset, pieceLength - 1));
    }
