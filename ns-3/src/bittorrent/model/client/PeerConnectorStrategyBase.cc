...
{
  NS_LOG_INFO ("PeerConnectorStrategyBase: Contacting tracker to join cloud...");
...
{
  NS_LOG_INFO ("PeerConnectorStrategyBase: Contacting tracker to report completed download.");
...
{
  NS_LOG_INFO ("PeerConnectorStrategyBase: Disconnecting all peers and leaving cloud.");
...
void PeerConnectorStrategyBase::StartListening (uint16_t port)
{
  ...
  m_serverSocket->Bind (InetSocketAddress (port));
...
{
  NS_LOG_INFO ("PeerConnectorStrategyBase: Explicitly connecting to " << address << ":" << port << ".");
...
void PeerConnectorStrategyBase::DisconnectPeers (int32_t count)
{
  if (count == -1)      // Close all connections
    {
      NS_LOG_INFO ("PeerConnectorStrategyBase: Disconnecting all peers.");
...
void PeerConnectorStrategyBase::ParseResponse (std::istream &response)
{
  ...
  NS_LOG_DEBUG ("PeerConnectorStrategyBase: ParseResponse -> Parsing complete");
  ...
  if (errorMsg)
    {
      ...
      NS_LOG_DEBUG ("PeerConnectorStrategyBase: ParseResponse -> Error");
      return;
    }
  ...
  NS_LOG_DEBUG ("PeerConnectorStrategyBase: ParseResponse -> Parsed renewal interval");
  ...
  NS_LOG_DEBUG ("PeerConnectorStrategyBase: ParseResponse -> Parsed incomplete number");
  // NS_ASSERT(leechers);
  leechers->GetData ();
  ...
  NS_LOG_DEBUG ("PeerConnectorStrategyBase: ParseResponse -> Parsed complete number");
  // NS_ASSERT(seeders);
  seeders->GetData ();
  ...
  NS_LOG_DEBUG ("PeerConnectorStrategyBase: ParseResponse -> Parsed tracker id");
  ...
  NS_LOG_DEBUG ("PeerConnectorStrategyBase: ParseResponse -> Parsed peers");
  // NS_ASSERT(peers);
      ...
      for (uint32_t peerNum = 0; peerNum < peerLen; peerNum += 2)
        {
          ...
          NS_LOG_DEBUG ("Peer received: " << peer.first << " at port " << peer.second);
  ...
  NS_LOG_INFO ( "Parsed a tracker response." );
}
...
void PeerConnectorStrategyBase::NewConnectionCreatedCallback (Ptr<Socket> sock, const  Address &addr)
{
  ...
  InetSocketAddress buf = InetSocketAddress::ConvertFrom (addr);
  Ptr<Peer> newPeer = CreateObject<Peer> (m_myClient);
//  newPeer->SetRemoteIp (buf.GetIpv4 ());
//  newPeer->SetRemotePort (buf.GetPort ());
      ...
      if (m_httpCC.HasBadHeader ())
        {
          ...
          NS_LOG_INFO ("PeerConnectorStrategyBase: Received error header from tracker.");
        }
...
bool PeerConnectorStrategyBase::ContactTracker (TrackerContactReason event, uint16_t numwant, std::map<std::string, std::string> additionalParameters, bool closeCurrentConnection)
{
  ...
  if (closeCurrentConnection)
  ...
  else
    {
      if (m_httpCC.HasBadHeader ())
        {
          NS_LOG_INFO ("PeerConnectorStrategyBase: Received bad header from tracker.");
